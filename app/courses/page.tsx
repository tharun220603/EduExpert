  ...
  return (
    ...
    >
      ...
      >
        ...
        <div>
          <h3 style={{ fontSize: "1.3rem", fontWeight: 800, color: "var(--text-primary)", marginBottom: "6px", letterSpacing: "-0.01em" }}>{course.name}</h3>
          <div style={{ fontSize: "0.75rem", color: "var(--text-muted)", fontWeight: 700, letterSpacing: "0.05em", textTransform: "uppercase" }}>{course.category}</div>
        </div>
        <div style={{ position: "absolute", top: "20px", right: "20px" }}>
            <span className="badge badge-amber" style={{ fontSize: "0.7rem", padding: "4px 10px", fontWeight: 700 }}>
                {course.duration}
            </span>
        </div>
      </div>
      ...
      <div style={{ padding: "32px 24px 24px", flex: 1, display: "flex", flexDirection: "column" }}>
        <p style={{ fontSize: "0.95rem", color: "var(--text-muted)", lineHeight: 1.7, marginBottom: "24px", display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
          {course.description}
        </p>

        <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginBottom: "24px" }}>
            {course.careerOptions.slice(0, 3).map((opt) => (
                <span key={opt} style={{ fontSize: "0.7rem", background: "#f8fafc", padding: "6px 14px", borderRadius: "10px", color: "var(--text-mid)", border: "1px solid var(--border)", fontWeight: 600 }}>
                    {opt}
                </span>
            ))}
        </div>
        ...
        <div style={{ marginTop: "auto", borderTop: "1px solid var(--border)", paddingTop: "24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
                <div style={{ fontSize: "0.65rem", color: "var(--text-muted)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.03em", marginBottom: "4px" }}>Avg Fees</div>
                <div style={{ fontWeight: 800, color: "var(--text-primary)", fontSize: "1.1rem" }}>{course.avgFees}</div>
            </div>
            <div 
              style={{ color: "var(--primary)", fontSize: "0.95rem", fontWeight: 700, display: "flex", alignItems: "center", gap: "4px" }}
            >
    ...
    <>
      <section className="page-hero" ref={heroRef}>
        <div className="container">
          <div className="page-hero-content">
            <span className="badge badge-amber">🎯 Course Finder</span>
            <h1>
              Explore <span>Career Courses</span>
            </h1>
            <p>
      ...
      <ScrollFloatSection className="section" speed={0.8}>
        <div className="container" ref={containerRef}>
          ...
          {/* Filters */}
          <div className="filter-tabs" style={{ marginBottom: "40px", justifyContent: "flex-start" }}>
            {categories.map((cat) => (
              <button
                key={cat}
      ...
      <section className="section-sm">
        <div className="container">
          <div className="cta-banner cta-shimmer" ref={ctaRef}>
            <h3 style={{ color: "var(--text)", fontSize: "1.5rem", marginBottom: "8px" }}>
            ...
            <div style={{ display: "flex", gap: "12px", justifyContent: "center", flexWrap: "wrap" }}>
              <AntiGravityButton onClick={() => router.push("/colleges")} variant="primary">
                Browse Colleges <FiArrowRight />
              </AntiGravityButton>
              <AntiGravityButton onClick={() => router.push("/exams")} variant="ghost">
                Check Exam Dates
              </AntiGravityButton>
            </div>
          </div>
